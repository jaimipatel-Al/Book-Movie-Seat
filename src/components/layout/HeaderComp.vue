...
<template>
  <header class="pay-5 pax-10 flex justify-between">
    <div class="flex r-space-x-3">
      <div class="flex items-center r-text-3xl cursor-pointer text-gray-600" @click="router.push('/')">
    ...
    <div class="flex r-space-x-3 items-center">
      <div v-if="isLogin" class="flex items-center cursor-pointer" @click="router.push('/user/profile')">
        <UserIcon class="r-w-10 p-1 rounded-full border border-gray-400 text-gray-400 mx-1 md:mx-2" />
      ...
      <MagnifyingGlassIcon class="block sm:hidden w-6 md:w-8 cursor-pointer" @click="openSearch()" />
          ...
          <li>
            <h2 class="r-text-2xl">
              <p class="flex items-center cursor-pointer">
                <ChevronLeftIcon class="block sm:hidden w-6 p-1 hover:bg-slate-200 rounded-full mr-1" />
                Hey{{ isLogin ? "," : "!" }}
              </p>
              <p v-if="isLogin"
            ...
            <UserIcon v-if="isLogin" class="r-w-10 p-1 rounded-full border border-gray-400 text-gray-400 mx-2" />
          </li>
          <li v-if="!isLogin">
    ...
    <div class="searchbar open" id="searchbar">
